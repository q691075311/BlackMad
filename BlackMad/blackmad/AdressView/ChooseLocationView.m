...
- (void)setUp{
    ...
    titleLabel.textColor = COLORWITHRGB(74, 74, 74);
    ...
    _underLine.backgroundColor = COLORWITHRGB(203, 50, 50);
...
- (void)addTableView{
    ...
    //TODO:全局变量self.tableViews数组保存所有创建的tableView
    [self.tableViews addObject:tabbleView];
    tabbleView.tableHeaderView = [self separateLine];
...
//MARK: --地址(AddressView)_topTabbar上添加标题
- (void)addTopBarItem{
    
    UIButton * topBarItem = [UIButton buttonWithType:UIButtonTypeCustom];
    [topBarItem setTitle:@"请选择" forState:UIControlStateNormal];
    [topBarItem setTitleColor:COLORWITHRGB(203, 50, 50) forState:UIControlStateNormal];
    [topBarItem setTitleColor:COLORWITHRGB(203, 50, 50) forState:UIControlStateSelected];
    [topBarItem sizeToFit];
     topBarItem.centerY = _topTabbar.height * 0.5;
    topBarItem.titleLabel.font = [UIFont systemFontOfSize:16];
    //TODO:全局变量self.topTabbarItems数组保存所有创建的地址按钮topBarItem
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    /**
     *  [tableView indexPathForSelectedRow]即将选中，还未选中，所以这个记录的是还是之前选中的indexPath
     */
//     NSIndexPath * indexPathWill = [tableView indexPathForSelectedRow];
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        //1.1 判断是否是第一次选择,不是,则重新选择省,切换省. 返回nil或者indexPath 第一次选择为nil
        /**
         *  返回选定行的indexPath,单行
         */
        NSIndexPath * indexPath0 = [tableView indexPathForSelectedRow];

        //比较当前indexPath与选中的是否一样    判断如果indexPath0存在（不是第一次选中）并且不相等
        ...
        //TODO:--之前未选中省，第一次选择省第一次即将选中，indexPath为nil
        [self addTopBarItem];
        [self addTableView];
        ...
        //跟上次数据一样，所以没有remove，也没有添加，直接切换scrollView 的偏移量，及改变指示条位置
            [self scrollToNextItem:cityItem.name];
            return indexPath;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    // 记录的是刚点击的选中的indexPath
//     NSIndexPath * indexPathDid = [tableView indexPathForSelectedRow];
    AddressItem * item;
    if([self.tableViews indexOfObject:tableView] == 0){
       item = self.dataSouce[indexPath.row];
    }else if ([self.tableViews indexOfObject:tableView] == 1){
...
//MARK:完成地址选择,执行chooseFinish代码块    首页显示选中之后的地址
